...
- `setItems(items: ICard[]): void` - добавляет в `products` карточки товаров, данные которых приходят с сервера
...
- `add(item: IProduct): void` - добавляет в корзину товар
...
- `_payment: Payment` - способ оплаты, который выбрал пользователь
- `_address: string` - адрес доставки
- `valid: boolean` - флаг валидности формы
- `errors: IOrder ` - объект со значениями полей `_payment` и `_address`

Методы:

- `checkValidation(): void` - метод валидации формы
- `checkAddress(value: string): string` - метод для валидации адреса доставки, возвращает текст ошибки, если она есть
- `checkPayment(value: string): string` - метод для валидации способа оплаты, возвращает текст ошибки, если она есть
- `setField(data: { field: keyof IOrder, value: string })` - устанавливает значение в заданное поле.

Среди методов также есть геттеры и сеттеры адреса доставки и способо оплаты.
...
- `valid: boolean` - флаг валидности формы
- `errors: IContacts` - объект со значениями полей `_phone` и `_email`

Методы:

Повторяет методы класса `Order`, но для своих полей
...
Конструктор принимает темплейт контейнера корзины и экземпляр объекта `EventEmitter`.

Поля:

- `_content: HTMLElement` - контент корзины
- `_cartList: HTMLElement` - карточки товаров 
- `_orderButton: HTMLButtonElement` - кнопка `Оформить`
- `_totalPrice: HTMLSpanElement` - сумма товаров

Методы:

- `render(data: { content: HTMLElement[] }): HTMLElement ` - возвращает контейнер корзины с товарами.
...
- `render(): HTMLElemt` - вовращает контейнер модального окна и открывает его

### Класс SuccessModal

Предназначен для уведомления пользователя об успешной покупке.\
Конструктор принимает темплейт контента модального окна успешной покупки и экземпляр объекта `EventEmitter`.

Поля класса:

- `_content: HTMLElement` - контент модального окна
- `_price: HTMLSpanElement` - общая стоимость заказа
...
Предназначен для реализации форм, содержащих поля ввода.\
...
- `disableSubmitButton()` - изменяет активность кнопки сабмита на `false`
- `set valid(value: boolean): void` - изменяет активность кнопки сабмита
- `render(data: object): HTMLElement` - возвращает контейнер формы и показывает его пользователю


### Класс OrderForm

Предназначен для реализации формы с данными доставки заказа. Наследуется от класса `Form`.\
Конструктор принимает темплейт контента формы и экземпляр объекта `EventEmitter`.

Методы:

- `disableButtons()` - сбрасывает выбор способа оплаты
...
В конструктор класса передается DOM-элемент темплейта, экземпляр класса `EventEmitter` для инициации событий и объект с функцией-слушателем.\
...
Конструктор принимает DOM-элемент темплейта превью карточки, экземпляр класса `EventEmitter` для инициации событий и объект с функцией-слушателем.
...
Конструктор принимает DOM-элемент темплейта карточки товара в корзине, экземпляр класса `EventEmitter` для инициации событий и объект с функцией-слушателем.
...
- `render()` - возвращает контейнер хедера
...
- `_header: HTMLElement` - хедер главной страницы
- `_catalog: HTMLElement` - каталог с карточками товаров
- `_locked: boolean` - флаг доступа прокрутки страницы
...
- `order:success` - закрывает модальное окно с подтверждением успешной покупки.
- `orderInput:changed` - пользователь вводит данные в форму с данными доставки, что приводит к появлению ошибок (если пользователь\
вводит некорректные данные)
- `contactsInput:changed` - пользователь вводит данные в форму с данными доставки, что приводит к появлению ошибок (если пользователь\
вводит некорректные данные)
- `modal:close` - пользователь закрывает модальное окно, главную страницу снова можно листать